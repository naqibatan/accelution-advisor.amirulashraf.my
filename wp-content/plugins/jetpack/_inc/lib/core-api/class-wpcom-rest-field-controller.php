...
abstract class WPCOM_REST_API_V2_Field_Controller {
	...
	public function __construct() {
		if ( ! $this->object_type ) {
			...
			_doing_it_wrong( 'WPCOM_REST_API_V2_Field_Controller::$object_type', sprintf( esc_html__( "Property '%s' must be overridden.", 'jetpack' ), 'object_type' ), 'Jetpack 6.8' );
			return;
		}

		if ( ! $this->field_name ) {
			/* translators: %s: field_name */
			_doing_it_wrong( 'WPCOM_REST_API_V2_Field_Controller::$field_name', sprintf( esc_html__( "Property '%s' must be overridden.", 'jetpack' ), 'field_name' ), 'Jetpack 6.8' );
			return;
		}
	...
	final public function get_for_response( $object_data, $field_name, $request, $object_type ) {
		...
		if ( ! $permission_check ) {
			...
			_doing_it_wrong( 'WPCOM_REST_API_V2_Field_Controller::get_permission_check', sprintf( esc_html__( "Method '%s' must return either true or WP_Error.", 'jetpack' ), 'get_permission_check' ), 'Jetpack 6.8' );
	...
	final public function update_from_request( $value, $object_data, $field_name, $request, $object_type ) {
		...
		if ( ! $permission_check ) {
			...
			_doing_it_wrong( 'WPCOM_REST_API_V2_Field_Controller::update_permission_check', sprintf( esc_html__( "Method '%s' must return either true or WP_Error.", 'jetpack' ), 'update_permission_check' ), 'Jetpack 6.8' );
	...
	public function get_permission_check( $object_data, $request ) {
		/* translators: %s: get_permission_check() */
		_doing_it_wrong( 'WPCOM_REST_API_V2_Field_Controller::get_permission_check', sprintf( esc_html__( "Method '%s' must be overridden.", 'jetpack' ), __METHOD__ ), 'Jetpack 6.8' );
	}
	...
	public function get( $object_data, $request ) {
		/* translators: %s: get() */
		_doing_it_wrong( 'WPCOM_REST_API_V2_Field_Controller::get', sprintf( esc_html__( "Method '%s' must be overridden.", 'jetpack' ), __METHOD__ ), 'Jetpack 6.8' );
	}
	...
	public function update_permission_check( $value, $object_data, $request ) {
		/* translators: %s: update_permission_check() */
		_doing_it_wrong( 'WPCOM_REST_API_V2_Field_Controller::update_permission_check', sprintf( esc_html__( "Method '%s' must be overridden.", 'jetpack' ), __METHOD__ ), 'Jetpack 6.8' );
	}
	...
	public function update( $value, $object_data, $request ) {
		...
		_doing_it_wrong( 'WPCOM_REST_API_V2_Field_Controller::update', sprintf( esc_html__( "Method '%s' must be overridden.", 'jetpack' ), __METHOD__ ), 'Jetpack 6.8' );
	}
	...
	public function get_schema() {
		...
		_doing_it_wrong( 'WPCOM_REST_API_V2_Field_Controller::get_schema', sprintf( esc_html__( "Method '%s' must be overridden.", 'jetpack' ), __METHOD__ ), 'Jetpack 6.8' );
	}
